...
The code itself consists of two Python packages. CudaVox is a python package that provide bindings to c++ code using pybind11 and xtensor python (using CUDA to perform calculations on a GPU and OpenMP to perfrom them in parallel on a multi-core CPU). Cad2vox meanwhile is a pure python package that reads in and wrangles the mesh/greyscale data using meshio and acts as a user interface to Cudavox.
...
Once you have the dependencies installed you can use the setup.py scripts to build and install the three packages as:
...
The main facing python function from Cad2Vox is voxelise. CudaVox defaults to performing the voxelisation on the GPU by first checking for a comptible GPU. If it can't find one (or the package was built without CUDA) it automatically switches to running on the CPU as a fallback. If using the CPU it will automatically detect and use the maxium number of threads avalible you can overide this by setting the enviroment variable OMP_NUM_THREADS to the number of threads required.

The following can be viewed at anytime through python by calling:

```python
import cad2vox
help(cad2vox.voxelise)
```
voxelise(input_file, output_file, greyscale_file=None, gridsize=0, unit_length=-1.0, use_tetra=False, cpu=False, solid=False)
	
    Wrapper Function to setup the CudaVox python bindings for the C++ code and provide the main user
    interface.

    This function will first try to perform the voxelisation using a CUDA capible GPU. If that fails or CUDA is unavalible
    it will fallback to running on CPU with the maximum number of avalible threads.
    ...
    simply skips the CUDA check altogther and permenantly runs on CPU.
